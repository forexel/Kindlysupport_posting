...
export function PublicationsPage() {
  ...
  const filtered = useMemo(() => {
    return posts.filter((p) => {
      ...
      const channel = String(p.preview_payload?.published?.channels?.[0] || 'telegram');
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2 text-sm text-zinc-400">
        <span>Главная</span>
        <span>/</span>
        <span className="text-zinc-200">Публикации</span>
      </div>
      ...
      <div className="flex flex-col gap-4 sm:flex-row bg-zinc-900 p-4 rounded-lg border border-zinc-800">
        ...
        <Select value={channelFilter} onValueChange={setChannelFilter}>
          ...
          <SelectContent className="bg-zinc-900 border-zinc-800">
            ...
            <SelectItem value="vk_channel" className="text-zinc-200">VK Channel</SelectItem>
      ...
      <div className="bg-zinc-900 rounded-lg border border-zinc-800 overflow-hidden">
        <Table>
          ...
          <TableBody>
            {filtered.length === 0 ? (
              <TableRow>
                <TableCell colSpan={4} className="text-center py-12 text-zinc-500">Публикации не найдены</TableCell>
              </TableRow>
            ) : filtered.map((pub) => (
              <TableRow
                key={pub.id}
